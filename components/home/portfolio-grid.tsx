...
const DUMMY_BOOKS = [
  { id: "1", title: "The Last Horizon", author: "Sarah Mitchell", coverUrl: "/images/books/cover-1.png" },
  { id: "2", title: "Beyond The Pages", author: "James Carter", coverUrl: "/images/books/cover-2.png" },
  { id: "3", title: "Whispers of Ink", author: "Elena Woods", coverUrl: "/images/books/cover-3.png" },
  { id: "4", title: "Rising Tides", author: "Marcus Hall", coverUrl: "/images/books/cover-4.png" },
  { id: "5", title: "Silent Echoes", author: "Diana Cross", coverUrl: "/images/books/cover-5.png" },
  { id: "6", title: "The Garden Path", author: "Thomas Reed", coverUrl: "/images/books/cover-6.png" },
  { id: "7", title: "Paper Hearts", author: "Lily Chen", coverUrl: "/images/books/cover-7.png" },
  { id: "8", title: "Neon Nights", author: "Jake Stone", coverUrl: "/images/books/cover-8.png" },
]
  ...
  return (
    ...
    >
      <div className="relative group">
        ...
        <div className="mt-4 text-center" style={{ opacity: clamp(1 - abs * 2, 0, 1) }}>
          <p className="text-primary-950 font-heading font-bold text-sm truncate">{book.title}</p>
...
export function PortfolioGrid({ books = [] }: PortfolioGridProps) {
  const displayBooks = books.length > 0 ? books.map(b => ({
    id: b._id,
    title: b.title,
    author: b.author,
    coverUrl: b.coverImage ? urlForImage(b.coverImage)?.url() : "/images/books/cover-1.png"
  })) : DUMMY_BOOKS
  ...
  const onPointerUp = () => {
    ...
    const maxJump = 2
    const clamped = Math.round(clamp(snapped - Math.round(current), -maxJump, maxJump)) + Math.round(current)
    animate(x, clamped, { type: "spring", stiffness: 300, damping: 35, mass: 0.8 })
  }
  ...
  const goTo = (idx: number) => {
    ...
    if (delta < -count / 2) delta += count
    animate(x, x.get() - delta, { type: "spring", stiffness: 250, damping: 32 })
  }
  ...
  return (
    <Section spacing="lg" className="bg-white overflow-hidden">
      <Container>
        <div className="text-center max-w-2xl mx-auto mb-16 flex flex-col items-center">
          ...
          <Text size="lg" className="text-primary-600/70">
            A showcase of books we have helped bring to life — from concept to publication.
          </Text>
        </div>
      </Container>
      ...
      <div className="relative w-full" style={{ perspective: "1200px" }}>
        <div
          className="relative h-[420px] mx-auto select-none touch-pan-y cursor-grab active:cursor-grabbing"
        ...
        >
          {slots.map((slot) => {
            ...
            const realOffset = renderPos + mod(Math.round(-renderPos) + slot, count)
